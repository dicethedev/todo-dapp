import { contractAddress, contractABI, connectWallet } from './getAccount';
...
async function createTaskList() {
    try {
        ...
        contract = new ethers.Contract(contractABI, contractAddress);
        try {
            numberOfTask = await contract.methods.getTaskCount().call({ from: etherjs.defaultAccount });
            console.log('Number of Tasks are ' + numberOfTask);
            // If there are task present //
            if (numberOfTask != 0) {
                // Fetch one task after the other until no task remain //
                console.log('Start fetching task now ...');
                let taskIterator = 0;

                while (taskIterator < numberOfTask) {
                    try {
                        let task = await contract.methods.getTask(taskIterator).call({ from: etherjs.defaultAccount });
                        if (task[0] != '') {
                ...
                // this will update the task count in HTML //
                updateTasksCount();
            }
        } catch {
...
/**
 * @author Blessing Samuel <https://github.com/dicetheddev>
 * @param {number} id
 * @param {string} name
 ...
 * @author Blessing Samuel <https://github.com/dicethedev>
...
async function removeTask(taskIndex) {
    ...
    try {
        await contract.methods.deleteTask(taskIndex).send({ from: etherjs.defaultAccount });
...
async function changeTaskStatus(id, taskIndex) {
    ...
    try {
        await contract.methods.updateStatus(taskIndex, checkbox.checked).send({ from: etherjs.defaultAccount });
...
function updateTasksCount() {
    // Getting the element of ul tag //
    let list = document.getElementById('list');
    // Getting the count of the ul element //
...
/**
 * addTask() add the task to the HTML via addTasktoList() and then
...
async function addTask(name) {
    ...
    if (form.checkValidity()) {
     ...
     //    document.getElementById('connectBtn-new-task', connectWallet).value = '';
        ...
        contract.methods.getTaskCount().call({ from: ethersjs.defaultAccount }).then(numberOfTask => {
